...
class ZPad3D(nn.Module):
    def __init__(self, padding, mode="reflect"):
        ...
        self.padding = torch.nn.modules.utils._ntuple(
            4)((padding, padding, 0, 0))
        self.mode = mode

    def forward(self, x):
        if x.shape[0] != 1:
            raise NotImplementedError(
                "TODO: 5D padding with batch size > 1 not implemented")
...
class XYPad3D(nn.Module):
    def __init__(self, padding, mode="constant"):
        ...
        self.padding = torch.nn.modules.utils._ntuple(4)(
...
class PartialConv2d(nn.Conv2d):
    ...
    def forward(self, x, mask, return_mask=False):
        ...
        if self.bias is None:
            output = torch.mul(super(PartialConv2d, self).forward(
                torch.mul(x, mask)), self.inv_support)
        else:
            bias = self.bias.view(1, self.out_channels, 1, 1)
            output = torch.mul(super(PartialConv2d, self).forward(
                torch.mul(x, mask)) - bias, self.mask_ratio) + bias
            output = torch.mul(output, self.update_mask)

        # don't change values inside original mask
        output[mask > 0.5] = X[mask > 0.5]
...
class InplaneLowpassFilter3D(nn.Module):
    def __init__(self, channels=1, param=3, kernel_size=9, mode="reflect", filter_type="sinc-hamming", **kwargs):
        ...
        if filter_type.lower() == "gaussian":
            ...
            kernel = torch.sin(2 * math.pi * sinc_bw * t) / \
                (2 * math.pi * sinc_bw * t)
            ...
            if filter_type.endswith("hamming"):
                ...
                if ks > 1:
                    kernel *= 0.54 - 0.46 * \
                        torch.cos((2 * math.pi * (t + ks // 2)) / (ks - 1))
    ...
    def forward(self, x):
        ...
        xp = F.pad(x.view(shape[0] * shape[1], *shape[2:]),
                   self.padding, self.mode)  # pad y
        xp = F.pad(
            xp.view(1, shape[0] * shape[1], shape[2], -1), self.padding, self.mode)  # pad x
...
class InplaneGaussian(nn.Module):
    ...
    def __init__(self, channels=1, blur_sigma=3, kernel_size=9, mode="reflect"):
        ...
        gaussian_kernel = gaussian_kernel / \
            torch.sum(gaussian_kernel, dim=[0, 1])
        gaussian_kernel = gaussian_kernel.view(
            1, 1, kernel_size, kernel_size, Z)
...
class MaskFill3D(nn.Conv3d):
    def __init__(self, *args, **kwargs):
        ...
        # mask filter:
        self.weight_mask = torch.ones(
            1, 1, ks[0], ks[1], ks[2])  # * self.patch_size

        # filter to fill values:
        filter_type = kwargs.pop('type', 'rbf')
        self.bias = None
        if filter_type == 'average':
            # = torch.ones(1, self.in_channels, *ks)
            self.weight.data.fill_(1.0 / self.patch_size)
            ...
            distance_sq = (x**2)[:, None, None] + \
                (y**2)[None, :, None] + (z**2)[None, None, :]
            rbf = np.exp(distance_sq/(-2*sigma**2))
            rbf /= rbf.sum()
            self.weight.data = torch.from_numpy(
                rbf.astype(np.float32)).view(1, 1, *rbf.shape)
        else:
            assert 0, filter_type
    ...
    def forward(self, x, mask, return_mask=True):
        ...
        with torch.no_grad():
            ...
            self.update_mask = F.conv3d(mask, self.weight_mask, bias=None, stride=self.stride,
                                        padding=self.padding, dilation=self.dilation, groups=1)
            ...
            if self.bias is None:
                output = torch.mul(super(MaskFill3D, self).forward(
                    torch.mul(x, mask)), self.inv_support)
            else:
                bias = self.bias.view(1, self.out_channels, 1, 1)
                output = torch.mul(super(MaskFill3D, self).forward(
...
class SliceLowPassFilter(nn.Module):
    def __init__(self, channels=1, kernel_size=9, padding="reflect", filter_type="gaussian", gauss_std=1, sinc_bw=0.25):
        ...
        if self.type.startswith("sinc"):
            ...
            mean = (kernel_size - 1) / 2.
            variance = sigma ** 2.
            kernel = torch.exp(-(coord.float() - mean).double()
                               ** 2. / (2. * variance))
        else:
            assert 0, self.type
    ...
    def forward(self, x):
        ...
        p = torch.nn.modules.utils._ntuple(4)(
            (self.padding, self.padding, 0, 0))
        if self.padding_mode == "replicate":  # replicate padding is implemented for 4D input
            xp = F.pad(x.view(x.shape[0] * x.shape[1], *x.shape[2:]),
                       p, self.padding_mode).view(*shape_padded)
        else:  # reshape to b * c, xy, z
            xp = F.pad(x.view(1, shape[0] * shape[1], -1, shape[-1]),
                       p, self.padding_mode).view(*shape_padded)
        return self.conv(xp)
...
class WeightedSliceFilter(nn.Module):
    def __init__(self, channels=1, kernel_size=9, kernel_width=1, weight_smooth=6, padding="reflect",
        ...
        self.conv = nn.Conv3d(in_channels=channels, out_channels=channels,
                              kernel_size=(
                                  kernel_width, kernel_width, kernel_size),
                              stride=1, padding=0, groups=channels, bias=False)
        self.conv.weight.data = kernel.repeat(channels, 1, 1, 1, 1)
        # kernel.expand_as(gaussian.weight.data) changes stride of dim 0
        self.conv.weight.requires_grad = False
        ...
        torch.nn.init.constant_(
            self.mask_conv.weight, 1.0 / (kernel_width * kernel_width * kernel_size))
        ...
        if weight_smooth:
            ...
            assert ks % 2 == 1, ks
            self.weight_filter = InplaneGaussian(
                channels=1, blur_sigma=weight_smooth, kernel_size=ks, mode="reflect")
    ...
    def forward(self, x, mask=None, outside_fill=1e-3, ret_support=False):
        ...
        mp = F.pad(mask.view(
            mask.shape[0] * mask.shape[1], *mask.shape[2:]), p, 'constant', outside_fill)
...
class SeparableConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, bias=False, preventRedundantParams=False):
        ...
        if kernel_size > 1 or not preventRedundantParams:
            self.conv = nn.Conv3d(in_channels, in_channels, kernel_size,
...
class StackedZDilatedSeparableConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, zdilations=[1, 2, 3], bias=False, grouped=True):
        ...
        for idal, dilation in enumerate(self.zdilations):
            setattr(self, 'conv%i' % idal, nn.Conv3d(in_channels, in_channels, kernel_size, stride, [
                    padding, padding, padding+dilation-1], [1, 1, dilation], groups=groups, bias=bias))
        self.pointwise = nn.Conv3d(
            in_channels*len(self.zdilations), out_channels, 1, 1, 0, 1, 1, bias=bias)

    def forward(self, x):
        x = torch.cat([getattr(self, 'conv%i' % idal)(x)
                       for idal in range(len(self.zdilations))], 1)
        x = self.pointwise(x)
        return x
...
class StackedMirroredZDilatedSeparableConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([getattr(self, 'conv%i' % idal)(
            getattr(self, 'pad%i' % idal)(x)) for idal in range(len(self.zdilations))], 1)
        x = self.pointwise(x)
        return x
...
class ConvBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, stride,
        ...
        norm_dim = output_dim
        if norm == 'bn':
            # TODO inplace activation and BN: https://github.com/mapillary/inplace_abn
...
class ResBlock(nn.Module):
    def __init__(self, dim, norm='in', activation='relu', pad_type='zero', n_conv=2):
        ...
        for i in range(n_conv - 1):
            model += [ConvBlock(dim, dim, 3, 1, 1, norm=norm,
    ...
    def __init__(self, num_blocks, dim, norm='in', activation='relu', pad_type='zero', res_n_conv=2, se_reduction=4):
        ...
        for i in range(num_blocks):
            self.model += [ResBlock(dim, norm=norm, activation=activation,
                                    pad_type=pad_type, n_conv=res_n_conv)]
