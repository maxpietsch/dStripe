...
class SNet3(nn.Module):
    def __init__(self, n_block, input_dim, dim=16, updim=2, downsample=None, final_pool=2,
        ...
        if isinstance(mode, int):
            modes = {0: 'global_avpool', 1: 'upsample', 2: 'center', 3: 'center+', 4: 'none'}
        ...
        if field_filter is None:
            ...
            ff_opt = {"kernel_size": 9, "padding": 'reflect'}  # TODO explicitly set "filter_type": "sinc_hamming", left as is due to legacy bug
            ...
            ff_opt = {"kernel_size": 9, "padding": 'reflect', "filter_type": "gaussian"}
            ff_opt.update(field_filter_options)
            self.field_filter = SliceLowPassFilter(channels=1, **ff_opt)
        else:
        ...
        self.model += [SeparableConv3d(input_dim, dim, kernel_size=k1, stride=1, padding=k1//2, dilation=1, bias=bias)]
        dim0 = dim
        for iblock in range(1, n_block):
            if dobn:
                self.model += [nn.BatchNorm3d(dim0, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)]
            self.model += [ConvBlock(dim0, dim, k1, 1, k1//2, norm=norm, activation='none', pad_type=pad_type, zdilation=zdilation)]
            self.model += [ConvBlock(dim, dim, k1, 1, k1//2, norm=norm, activation=activ, pad_type=pad_type, zdilation=zdilation)]
            dim0 = dim
        ...
        if final_pool is not None:
            self.model += [AdaptiveConcatPool3d(sz=(final_pool, final_pool, None))]
            dim0 = 2 * dim
    ...
    def forward(self, x, eps=1.0e-4):
        ...
        if self.mode == 'global_avpool':
            ...
            p = nn.Upsample(size=shp[-3:], mode='trilinear', align_corners=False)
        elif self.mode == 'center':
            p = lambda im: im[..., im.shape[-3]//2, im.shape[-2]//2, :]
        elif self.mode == 'center+':
            def p(im):
                cx = im.shape[-3] // 2
                cy = im.shape[-2] // 2
        ...
        # https://github.com/ndrplz/ConvLSTM_pytorch/issues
        field = self.model(x)
        field = 2.0 * torch.sigmoid(field) + eps  # restrict to [eps, 2+eps], 0 becomes 1.0+eps
        ...
        if self.field_filter is not None:
            field = torch.exp(torch.log(field) - self.field_filter(torch.log(field)))
